...
class ConvGRUMultitask(nn.Module):
    def __init__(self,dynamic_channels:int=6,static_channels:int=13,targets:int=5,hidden_channels:int=32,decoder_channels:int=32,output_lower:list[float]|None=None,output_upper:list[float]|None=None)->None:
        super().__init__(); self.encoder=ConvGRU(dynamic_channels,hidden_channels); self.stem=nn.Conv2d(hidden_channels+dynamic_channels+static_channels,decoder_channels,3,padding=1); self.trunk=nn.Sequential(ResidualBlock(decoder_channels),ResidualBlock(decoder_channels)); self.heads=nn.ModuleList([nn.Conv2d(decoder_channels,1,1) for _ in range(targets)])
    ...
    def forward(self,sequence:torch.Tensor,current:torch.Tensor,static:torch.Tensor)->torch.Tensor:
        hidden=self.encoder(sequence); features=self.trunk(self.stem(torch.cat((hidden,current,static),1))); raw=torch.cat([head(features) for head in self.heads],1)
