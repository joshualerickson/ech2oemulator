...
def main() -> None:
 ...
 for path in sorted(a.daily_dir.glob('*_daily_screen.csv')):
  ...
  for target,record in by.items():
   ...
   rows.append({'site_id':record['site_id'],'bbox_id':record['bbox_id'],'water_year_end':record['water_year_end'],'temporal_contract':TEMPORAL_CONTRACT,'start_date':history[0]['date'],'end_date':record['date'],'target_date':record['date'],'sequence_length':a.sequence_length,'target_time_index':record['target_time_index'],'forcing_start_index':history[0]['forcing_time_index'],'forcing_end_index':record['forcing_time_index']})
