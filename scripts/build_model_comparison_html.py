...
<h2>Comparison table</h2><div id="comparison"></div><h2>Channel-mask metrics</h2><p class="muted">Pooled June–September validation pixels inside versus outside each site-local <code>chanmask.map</code>; mask nodata is excluded from both domains.</p><div id="channelComparison"></div><h2>Available reports</h2><div class="grid" id="cards"></div>
...
document.querySelectorAll('select').forEach(x=>x.addEventListener('change',render));addEventListener('resize',render);render();</script></body></html>"""
    ...
    template = template.replace(
        "document.querySelector('#comparison').innerHTML=text+'</tbody></table>';let cards=",
        "document.querySelector('#comparison').innerHTML=text+'</tbody></table>';let channel='<table><thead><tr><th>Model / domain</th>'+targets.map(t=>'<th>'+t+'</th>').join('')+'</tr></thead><tbody>';for(const label of labels){const domains=reports[label]?.channel_mask_by_domain||{};for(const domain of ['inside_channel','outside_channel']){channel+='<tr><td>'+label+' / '+(domain==='inside_channel'?'inside channel':'outside channel')+'</td>';for(const t of targets){const x=domains[domain]?.[t];const shown=x&&x[metric]!==undefined?Number(x[metric]).toFixed(['mae','rmse','bias'].includes(metric)?4:3):'—';channel+='<td>'+shown+'</td>'}channel+='</tr>'}}document.querySelector('#channelComparison').innerHTML=channel+'</tbody></table>';let cards=",
    )
    template = template.replace(
