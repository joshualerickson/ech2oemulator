...
def main():
 ...
 for site in a.site_ids:
  ...
  for target,r in by.items():
   ...
   rows.append({'split':'dev_spatial_test','site_id':site,'bbox_id':site,'water_year_end':wy,'start_date':h[0]['date'],'end_date':r['date'],'target_date':r['date'],'sequence_length':30,'dynamic_sequence_source':str(root),'static_source':str(root/'Spatial'),'target_source':str(root/f'{site}-{wy}_subdaily.nc')})
