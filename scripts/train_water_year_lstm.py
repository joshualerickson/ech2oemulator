#!/usr/bin/env python3
"""Stateful Oct--Sep ConvLSTM training with TBPTT or full water-year BPTT."""
...
def model_config(stats):
 ...
 return {'recurrent_cell':'lstm','output_lower':lower,'output_upper':upper}
...
def site_loss_tbptt(model,s,stats,chunk,optim=None):
 ...
 for start in range(0,x.shape[1],chunk):
  end=min(x.shape[1],start+chunk);pred,state=model.forward_lstm_chunks(x[:,start:end],z,state);mask=valid[:,start:end]&days[start:end].view(1,-1,1,1,1)
  if mask.any():
...
def site_loss_full_bptt(model,s,stats,chunk,optim=None):
 ...
 for start in range(0,x.shape[1],chunk):
  end=min(x.shape[1],start+chunk);pred,state=model.forward_lstm_chunks(x[:,start:end],z,state);mask=valid[:,start:end]&days[start:end].view(1,-1,1,1,1);mask=mask & torch.isfinite(y[:,start:end])
  if mask.any():
