...
<h2>Current reading</h2><p>In the current fixed-window grid, the 90-day ConvLSTM is the strongest overall result. The full-water-year BPTT comparison includes 50-epoch continuation runs for both cells; each displayed continuation uses its validation-selected best checkpoint. These long-memory experiments are retained as explicit diagnostics rather than treated as direct winners over the different fixed-window cohort.</p>
