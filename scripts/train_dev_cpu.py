...
def site_loader(dataset:SequenceDataset,batch_size:int,seed:int,shuffle:bool)->DataLoader:
 """Use ordinary shuffled singleton batches or full-bbox site batches."""
 if batch_size==1: return DataLoader(dataset,batch_size=1,shuffle=shuffle,collate_fn=pad_collate,num_workers=0,generator=torch.Generator().manual_seed(seed))
 return DataLoader(dataset,batch_sampler=SiteBatchSampler(dataset,batch_size,seed,shuffle),collate_fn=pad_collate,num_workers=0)
...
def main():
 ...
 if a.benchmark_batches:
  ...
  for number,b in enumerate(loader,1):
   x=standardize(torch.nan_to_num(b['dynamic_sequence']),stats['dynamic'],DYNAMIC_CHANNELS,2); cur=standardize(torch.nan_to_num(b['current_forcing']),stats['dynamic'],DYNAMIC_CHANNELS,1); sta=standardize(torch.nan_to_num(b['static_stack']),stats['static'],STATIC_CHANNELS,1); target=standardize(b['target_stack'],stats['target'],TARGET_CHANNELS,1)
   loss=masked_huber(model(x,cur,sta),target,b['valid_mask'].bool()); opt.zero_grad(); loss.backward(); opt.step(); measured.append(float(loss.detach())); sample_count+=len(b['site_id'])
   if number>=a.benchmark_batches: break
  seconds=time.perf_counter()-started; print({'benchmark_batches':len(measured),'samples':sample_count,'seconds':seconds,'samples_per_second':sample_count/seconds,'mean_loss':sum(measured)/len(measured),'batch_size':a.batch_size,'threads':a.threads,'interop_threads':a.interop_threads},flush=True); return
 for epoch in range(start,a.epochs):
  epoch_started=time.perf_counter()
  epoch_losses=[]
  for b in loader:
   x=standardize(torch.nan_to_num(b['dynamic_sequence']),stats['dynamic'],DYNAMIC_CHANNELS,2); cur=standardize(torch.nan_to_num(b['current_forcing']),stats['dynamic'],DYNAMIC_CHANNELS,1); sta=standardize(torch.nan_to_num(b['static_stack']),stats['static'],STATIC_CHANNELS,1); target=standardize(b['target_stack'],stats['target'],TARGET_CHANNELS,1)
   loss=masked_huber(model(x,cur,sta),target,b['valid_mask'].bool()); opt.zero_grad(); loss.backward(); opt.step(); value=float(loss.detach()); losses.append(value); epoch_losses.append(value)
  validation_loss=None
  if val_loader is not None and (epoch+1)%a.validation_every==0:
   model.eval(); validation_losses=[]
   with torch.no_grad():
    for b in val_loader:
     x=standardize(torch.nan_to_num(b['dynamic_sequence']),stats['dynamic'],DYNAMIC_CHANNELS,2); cur=standardize(torch.nan_to_num(b['current_forcing']),stats['dynamic'],DYNAMIC_CHANNELS,1); sta=standardize(torch.nan_to_num(b['static_stack']),stats['static'],STATIC_CHANNELS,1); target=standardize(b['target_stack'],stats['target'],TARGET_CHANNELS,1); validation_losses.append(float(masked_huber(model(x,cur,sta),target,b['valid_mask'].bool())))
  ...
  payload={'model_state':model.state_dict(),'optimizer_state':opt.state_dict(),'epoch':epoch+1,'losses':losses,'epoch_metrics':epoch_metrics,'best_validation_loss':best_validation_loss,'parameter_count':sum(x.numel() for x in model.parameters()),'normalization':str(a.normalization),'model_config':model_config,'runtime_config':{'batch_size':a.batch_size,'threads':a.threads,'interop_threads':a.interop_threads},'seed':a.seed}; a.checkpoint.parent.mkdir(parents=True,exist_ok=True); torch.save(payload,a.checkpoint)
  if is_best: torch.save(payload,a.checkpoint.with_name(a.checkpoint.stem+'_best'+a.checkpoint.suffix))
  print({'epoch':epoch+1,'mean_loss':metric['train_mean_loss'],'validation_mean_loss':validation_loss,'last_batch_loss':epoch_losses[-1],'steps':len(losses),'epoch_seconds':time.perf_counter()-epoch_started},flush=True)
