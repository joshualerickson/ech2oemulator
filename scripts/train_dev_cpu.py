...
def main():
 p=argparse.ArgumentParser(); p.add_argument('--manifest',type=Path,required=True); p.add_argument('--normalization',type=Path,required=True); p.add_argument('--checkpoint',type=Path,required=True); p.add_argument('--train-split',default='dev_train',help='Manifest split used for optimization (default: dev_train).'); p.add_argument('--resume-from',type=Path,default=None,help='Resume this checkpoint into --checkpoint; --epochs becomes additional epochs.'); p.add_argument('--epochs',type=int,default=5); p.add_argument('--learning-rate',type=float,default=1e-3); p.add_argument('--threads',type=int,default=32); p.add_argument('--interop-threads',type=int,default=1); p.add_argument('--device',default='auto',help='auto (default), cpu, cuda, or cuda:N.'); p.add_argument('--batch-size',type=int,default=1,help='>1 batches same-site dates without spatial padding.'); p.add_argument('--benchmark-batches',type=int,default=0,help='Run this many train batches, report throughput, and exit without a checkpoint.'); p.add_argument('--recurrent-cell',choices=('gru','lstm'),default='gru',help='Use lstm only for a controlled ConvGRU comparison.'); p.add_argument('--seed',type=int,default=20260803); p.add_argument('--enforce-physical-bounds',action='store_true',help='Constrain soil moisture to [0,1] and PLC to [0,100] in the model head.'); p.add_argument('--validation-split',default=None,help='Optional site-level manifest split, e.g. dev_spatial_val.'); p.add_argument('--validation-every',type=int,default=1); p.add_argument('--validation-max-samples',type=int,default=0,help='0 validates every row of the split.'); p.add_argument('--early-stopping-patience',type=int,default=0,help='Stop after this many validation epochs without meaningful improvement; 0 disables it.'); p.add_argument('--min-delta',type=float,default=0.0,help='Required absolute validation-loss decrease to reset patience.'); p.add_argument('--resume',action='store_true',help='Legacy: resume --checkpoint through its total --epochs value.'); a=p.parse_args()
 if a.batch_size<1 or a.interop_threads<1 or a.early_stopping_patience<0 or a.min_delta<0: p.error('--batch-size, --interop-threads, --early-stopping-patience, and --min-delta must be non-negative (batch/thread values positive)')
 if a.resume and a.resume_from: p.error('Use either --resume or --resume-from, not both')
 if a.early_stopping_patience and not a.validation_split: p.error('--early-stopping-patience requires --validation-split')
 torch.set_num_threads(a.threads); torch.set_num_interop_threads(a.interop_threads); torch.manual_seed(a.seed); device=resolve_device(a.device); stats=json.loads(a.normalization.read_text())['groups']; ds=SequenceDataset(a.manifest,a.train_split,cache_site_arrays=True); loader=site_loader(ds,a.batch_size,a.seed,True,pin_memory=device.type=='cuda')
 val_loader=None
 ...
 model=ConvGRUMultitask(**model_config).to(device); opt=torch.optim.AdamW(model.parameters(),lr=a.learning_rate); start=0; losses=[]; epoch_metrics=[]; best_validation_loss=float('inf'); non_improving_epochs=0
 resume_path=a.resume_from if a.resume_from else a.checkpoint if a.resume else None
 if resume_path:
  if not resume_path.exists(): raise FileNotFoundError(resume_path)
  s=torch.load(resume_path,map_location='cpu',weights_only=False)
  ...
  load_model_state(model,s); opt.load_state_dict(s['optimizer_state']);
  for group in opt.param_groups: group['lr']=a.learning_rate
  start=s['epoch']; losses=s['losses']; epoch_metrics=s.get('epoch_metrics',[]); best_validation_loss=s.get('best_validation_loss',float('inf'))
  non_improving_epochs=s.get('non_improving_epochs',0)
  print({'resume_from':str(resume_path),'start_epoch':start,'resume_mode':'model_and_adamw_state','prior_best_validation_loss':best_validation_loss,'prior_non_improving_epochs':non_improving_epochs},flush=True)
 ...
 stop_epoch=start+a.epochs if a.resume_from else a.epochs
 for epoch in range(start,stop_epoch):
  ...
  is_best=validation_loss is not None and validation_loss<best_validation_loss-a.min_delta
  if validation_loss is not None:
   if is_best: best_validation_loss=validation_loss;non_improving_epochs=0
   else: non_improving_epochs+=1
  payload={'model_state':model.state_dict(),'optimizer_state':opt.state_dict(),'epoch':epoch+1,'losses':losses,'epoch_metrics':epoch_metrics,'best_validation_loss':best_validation_loss,'non_improving_epochs':non_improving_epochs,'early_stopping':{'patience':a.early_stopping_patience,'min_delta':a.min_delta},'parameter_count':sum(x.numel() for x in model.parameters()),'normalization':str(a.normalization),'model_config':model_config,'runtime_config':{'train_split':a.train_split,'batch_size':a.batch_size,'learning_rate':a.learning_rate,'threads':a.threads,'interop_threads':a.interop_threads,'device':str(device)},'resume_from':str(resume_path) if resume_path else None,'seed':a.seed}; a.checkpoint.parent.mkdir(parents=True,exist_ok=True); torch.save(payload,a.checkpoint)
  if is_best: torch.save(payload,a.checkpoint.with_name(a.checkpoint.stem+'_best'+a.checkpoint.suffix))
  print({'epoch':epoch+1,'mean_loss':metric['train_mean_loss'],'validation_mean_loss':validation_loss,'best_validation_loss':best_validation_loss,'non_improving_epochs':non_improving_epochs,'last_batch_loss':epoch_losses[-1],'steps':len(losses),'epoch_seconds':time.perf_counter()-epoch_started,'device':str(device)},flush=True)
  if a.early_stopping_patience and validation_loss is not None and non_improving_epochs>=a.early_stopping_patience:
   print({'early_stopping':'triggered','epoch':epoch+1,'best_validation_loss':best_validation_loss,'patience':a.early_stopping_patience,'min_delta':a.min_delta},flush=True);break
