...
def main():
 ...
 if a.validation_split:
  ...
  val_loader=DataLoader(val_ds,batch_size=1,shuffle=False,collate_fn=pad_collate,num_workers=0)
 model_config={}
 ...
 for epoch in range(start,a.epochs):
  ...
  payload={'model_state':model.state_dict(),'optimizer_state':opt.state_dict(),'epoch':epoch+1,'losses':losses,'epoch_metrics':epoch_metrics,'best_validation_loss':best_validation_loss,'parameter_count':sum(x.numel() for x in model.parameters()),'normalization':str(a.normalization),'model_config':model_config,'seed':a.seed}; a.checkpoint.parent.mkdir(parents=True,exist_ok=True); torch.save(payload,a.checkpoint)
  if is_best: torch.save(payload,a.checkpoint.with_name(a.checkpoint.stem+'_best'+a.checkpoint.suffix))
  print({'epoch':epoch+1,'mean_loss':metric['train_mean_loss'],'validation_mean_loss':validation_loss,'last_batch_loss':epoch_losses[-1],'steps':len(losses)},flush=True)
