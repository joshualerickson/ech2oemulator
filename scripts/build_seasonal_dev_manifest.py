 ...
 for split,sites in groups.items():
  for site in sites:
   ...
   for target,record in sorted(by.items()):
    ...
    rows.append({'split':split,'site_id':site,'bbox_id':site,'water_year_end':wy,'start_date':hist[0]['date'],'end_date':record['date'],'target_date':record['date'],'sequence_length':a.sequence_length,'dynamic_sequence_source':str(root),'static_source':str(root/'Spatial'),'target_source':str(root/f'{site}-{wy}_subdaily.nc')})
