...
def main():
 p=argparse.ArgumentParser();p.add_argument('--manifest',type=Path,required=True);p.add_argument('--checkpoint',type=Path,required=True);p.add_argument('--normalization',type=Path,required=True);p.add_argument('--site-id',required=True);p.add_argument('--month',type=int,default=8);p.add_argument('--max-days',type=int,default=1);p.add_argument('--chunk-days',type=int,default=30,help='Days replayed per loop; use 0 for one full water-year replay.');p.add_argument('--output-dir',type=Path,required=True);a=p.parse_args();stats=json.loads(a.normalization.read_text())['groups'];ds=WaterYearDataset(a.manifest,'dev_spatial_test');s=next(x for x in ds if x['site_id']==a.site_id);state=torch.load(a.checkpoint,map_location='cpu',weights_only=False);m=model_from_checkpoint(state);load_model_state(m,state);m.eval();x=standardize(torch.nan_to_num(s['dynamic']).unsqueeze(0),stats['dynamic'],DYNAMIC_CHANNELS,2);z=standardize(torch.nan_to_num(s['static']).unsqueeze(0),stats['static'],STATIC_CHANNELS,1);parts=[];hidden=None;chunk=x.shape[1] if a.chunk_days==0 else a.chunk_days
 with torch.no_grad():
